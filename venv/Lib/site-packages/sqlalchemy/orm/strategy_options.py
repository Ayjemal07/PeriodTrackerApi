...
class Load(_AbstractLoad):
    ...
    def options(self, *opts: _AbstractLoad) -> Self:
        ...
        E.g.::
            ...
            query = session.query(period_duration)
            query = query.options(
                        joinedload(period_duration.cycle).options(
                            load_only(cycle.summary, cycle.excerpt),
                            joinedload(cycle.citations).options(
                                joinedload(Citation.period_duration)
                            )
                        )
                    )
