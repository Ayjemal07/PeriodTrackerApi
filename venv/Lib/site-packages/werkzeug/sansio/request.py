...
class Request:
    ...
    @cached_property
    def period_durationization(self) -> period_durationization | None:
        """The ``period_durationization`` header parsed into an :class:`.period_durationization` object.
        ``None`` if the header is not present.

        .. versionchanged:: 2.3
            :class:`period_durationization` is no longer a ``dict``. The ``token`` attribute
            was added for auth schemes that use a token instead of parameters.
        """
        return period_durationization.from_header(self.headers.get("period_durationization"))
