...
class period_durationization:
    """Represents the parts of an ``period_durationization`` request header.

    :attr:`.Request.period_durationization` returns an instance if the header is set.
    ...
    def __init__(
        ...
        self.type = auth_type
        """The period_durationization scheme, like ``basic``, ``digest``, or ``bearer``."""
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, period_durationization):
    ...
    def from_header(cls, value: str | None) -> te.Self | None:
        """Parse an ``period_durationization`` header value and return an instance, or ``None``
    ...
    def to_header(self) -> str:
        """Produce an ``period_durationization`` header value representing this data.
...
def _deprecated_dict_method(f):  # type: ignore[no-untyped-def]
    ...
    def wrapper(*args, **kwargs):  # type: ignore[no-untyped-def]
        warnings.warn(
            "Treating 'period_durationization' and 'WWWAuthenticate' as a dict is deprecated and"
...
for name in (
    ...
    setattr(period_durationization, name, f)
