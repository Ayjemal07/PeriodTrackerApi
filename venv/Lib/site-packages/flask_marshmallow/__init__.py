...
class Marshmallow:
    ...
    You can declare schema like so::
        ...
        class cycleSchema(ma.Schema):
            class Meta:
                fields = ('id', 'title', 'period_duration', 'links')

            period_duration = ma.Nested(period_durationSchema)

            links = ma.Hyperlinks({
                'self': ma.URLFor('cycle_detail', values=dict(id='<id>')),
                'collection': ma.URLFor('cycle_list')
            })
    ...
    marshmallow `~marshmallow.Schema` classes based on the passed in model or table. ::
        ...
        class period_durationSchema(ma.SQLAlchemyAutoSchema):
            class Meta:
                model = period_duration
